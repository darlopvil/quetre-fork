...
/**
 * la busqueda ya no es scrapeable: el HTML de /search no trae resultados y
...
const getSearch = async (query, { after = null, type = 'all_types' } = {}) => {
  ...
  const rawData = res?.data?.searchConnection;

  if (!rawData) {
    const anonimo = res?.data?.viewer?.user === null;
    const e = new AppError(
      anonimo
        ? 'La búsqueda requiere una sesión de Quora configurada en esta instancia.'
        : 'No se pudo completar la búsqueda.',
      anonimo ? 501 : 502
    );
    e.code = anonimo ? 'SEARCH_NEEDS_SESSION' : 'EMPTY_PAYLOAD';
    throw e;
  }
